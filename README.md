...
- PDF: لا يوجد حد لعدد الطبقات؛ حجم الملف 30MB هو حد الرفع.
...
للصور وPDF يولد ملفات RGB/8-bit حقيقية، لكن اعتماد التوافق النهائي مع
Photoshop وAfter Effects يحتاج اختبارات Adobe Golden على الإصدارين
المستهدفين.

توجد الآن ملفات Golden حتمية وقائمة تحقق وتقرير تشغيل في
`artifacts/adobe-golden/`. نجح التوليد والفحص البرمجي، بينما يتوقف الفتح
الفعلي حاليًا على موافقة مسؤول ترخيص Adobe في بيئة الاختبار.
...
يتضمن corpus 71 صفحة من 16 كتابًا ملكية عامة: 6 مراجع community-proofread
و65 مرجعًا community-validated، مع development/validation/holdout معزولة
بحسب الكتاب. يستخدم المحرك استدراكًا حتميًا واحدًا كحد أقصى للصفحة منخفضة
الثقة، اعتمادًا على خصائص OCR والتخطيط لا اسم العينة، مع قص للهامش الأبيض
وتعويض الإحداثيات للصفحات القصيرة المزخرفة، ومرور إنجليزي محلي انتقائي
للصفحات المختلطة. حقق القياس الرسمي v4 نسبة CER كلية 16.97%، وvalidation
‏15.13%، وholdout مستقلًا 15.64% مع نجاح صفحاته العشر كلها. تبقى صفحة جدول
تاريخي واحدة من development عند 69.02%؛ لذلك يخرج القياس الرسمي برمز غير
صفري، بينما يلتقطها مسار `needs_review` لأن ثقتها النهائية 30.14% أقل من حد
السياسة 35%. ثُبتت بصمتان منفصلتان للتنفيذ ولمحتوى holdout، ويرفض القياس
أي انحراف لاحق. أي تغيير جديد في OCR أو قرار البوابة يتطلب holdout v5 من
مصادر جديدة.
...
حقيقية، واختيار الملف ينشئ المشروع وجلسة الرفع ثم يرسل البايتات، وخيار
`PSD` و`TIFF` و`PNG شفافة` و`PNG + JSON` داخل المراجعة النهائية تنشئ
الملفات وتنزّلها. تغييرات الاسم والرؤية والقفل والشفافية و`zIndex` وترتيب
