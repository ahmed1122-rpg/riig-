...
describe("character-rig HTTP foundation", () => {
  ...
  it("rejects identity training and image generation in source-only mode", async () => {
    const app = await harness.build(
      loadConfig({ NODE_ENV: "test", CHARACTER_RIG_ENABLED: "true" }),
    );
    const cookie = await registerCreator(app);
    ...
    const [bootstrap, generation] = await Promise.all([
      app.inject({
        method: "POST",
        url: `/v1/projects/${projectId}/character-rig/identity-model`,
        headers: { cookie, "x-idempotency-key": "identity-disabled-001" },
        payload: { bibleId: crypto.randomUUID() },
      }),
      app.inject({
        method: "POST",
        url: `/v1/projects/${projectId}/character-rig/generations`,
        headers: { cookie, "x-idempotency-key": "generation-disabled-001" },
        payload: {},
      }),
    ]);

    for (const response of [bootstrap, generation]) {
  ...
  it("downloads and idempotently approves verified compiled rig artifacts", async () => {
    ...
    const sourceBody = Buffer.from("source-image-fixture");
    const sourceMetadata = await objectStorage.put({
      key: `projects/${projectId}/character-rig/source.png`,
    ...
    const rig: CharacterRigVersion = {
      ...
      source: {
        ...
        artifact: artifact(sourceMetadata),
