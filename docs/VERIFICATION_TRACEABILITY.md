...
| `character-studio` | Character Studio is image-only and remains disabled until provider and product gates pass | capability contract, character worker, ADRs 0018–0019 | Character unit/integration, async provider, race, benchmark, and PSD tests | Private GPU inference provider, cold/warm load and cost evidence, Character Animator Golden, rights/legal approval | Character identity and rigging | `disabled-by-gate` |
| `character-gpu-serverless` | An enabled release uses bounded async-v1, same-origin polling, scale-to-zero, and release-bound provider evidence | `config/gpu-serverless-readiness-policy.json`, `CHARACTER_GPU_SERVERLESS.md` | provider contract tests, deployment verifier, provider-readiness and stable-promotion evidence validation | Selected GPU account/region/model, private networking, observed cold-start/load/cost and deletion evidence | Platform and Character identity | `external-pending` |
