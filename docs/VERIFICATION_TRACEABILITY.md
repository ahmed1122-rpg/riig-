...
| `character-studio` | Character Studio preserves the current uploaded image and its source layers without training, generation, face replacement, or invented views | capability contract, character worker, ADR 0020 | Source/reference/compiler/API/UI tests plus pixel-exact PSD comparison | Licensed Character Animator Golden evidence and production activation approval | Character source preparation and export | `source-verified` |
