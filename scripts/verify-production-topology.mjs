...
const registration = await api(0, "/v1/auth/register", {
  method: "POST",
  json: { name: "Topology Test", email, password },
  expectedStatus: 201,
});
...
const intent = await api(1, "/v1/uploads/intents", {
  ...
  headers: { "idempotency-key": `upload-${projectId}` },
  expectedStatus: 201,
});
...
const processing = await api(1, "/v1/processing/jobs", {
  ...
  headers: { "idempotency-key": `processing-${projectId}` },
  expectedStatus: 202,
});
...
await api(0, `/v1/projects/${projectId}/layer-document?sourceVersionId=${sourceVersionId}`, {
  expectedStatus: 200,
});
...
const exportJob = await api(0, "/v1/exports", {
  ...
  headers: { "idempotency-key": `export-${projectId}` },
  expectedStatus: 202,
});
